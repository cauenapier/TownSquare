...
const DEFAULT_ACTIVITY_REFRESH_MS = 10_000;
...
/**
 * Fold a set of public map sites into the small set of network numbers both
 * the homepage and the full map page want to show ("N towns, M active in the
 * last 30 days"). Kept pure so it is unit-testable without a DOM.
 *
 * @param {Array<{lastSeenAt?: number, activeVisitors?: number}>} sites
 * @param {number} [now]
 */
export function computeNetworkStats(sites, now = Date.now()) {
  const list = Array.isArray(sites) ? sites : [];
  let activeLast30d = 0;
  let visitorsNow = 0;

  for (const site of list) {
    const lastSeenAt = Number(site?.lastSeenAt) || 0;
    if (lastSeenAt > 0 && now - lastSeenAt <= ACTIVE_WINDOW_MS) activeLast30d += 1;
    visitorsNow += Math.max(0, Number(site?.activeVisitors) || 0);
  }

  return { towns: list.length, activeLast30d, visitorsNow };
}
...
/**
 * Tell the server a visitor followed a town's website link from a map, so the
 ...
 */
export function reportMapClick(siteKey, fetchBase = "") {
  ...
  try {
    ...
    navigator.sendBeacon(`${fetchBase}/api/map-click`, payload);
  } catch {
...
/**
 * Ask the server for another public town to visit, the same weighted draw the
 * widget's compass menu uses. Shared here so the homepage map, the full map
 * page, and the widget all agree on one behavior.
 *
 * @param {string} [currentSiteKey]
 * @param {string} [fetchBase] Origin to fetch against ("" for same-origin).
 */
export async function fetchRandomTown(currentSiteKey = "", fetchBase = "") {
  const url = new URL("/api/discovery/random", fetchBase || window.location.origin);
  if (currentSiteKey) url.searchParams.set("siteKey", currentSiteKey);
  const response = await fetch(url, { mode: "cors", credentials: "omit" });
  const body = await response.json();
  const destination = new URL(body?.town?.url || "", "invalid://");
  if (!response.ok || !["http:", "https:"].includes(destination.protocol)) return null;
  return { siteKey: body.town.siteKey, name: body.town.name || "Another TownSquare", url: destination.href };
}

/**
 * Mount an interactive TownSquare network map into `root`.
 *
 * `mode: "full"` (the /map page) gets pointer/wheel pan-zoom, matching the
 * page's existing behavior exactly. `mode: "embedded"` (the homepage) always
 * shows the whole world, never hijacks wheel/touch scrolling, and hides
 * lower-priority labels until a town is hovered/selected/focused so the
 * smaller canvas doesn't turn into label soup.
 *
 * @param {object} options
 * @param {HTMLElement} options.root Element the SVG is mounted into.
 ...
 * @param {(site: object|null) => void} [options.onSelect] Selection changes.
 * @param {(stats: {towns:number, activeLast30d:number, visitorsNow:number}) => void} [options.onStats]
 * @param {(text: string) => void} [options.onStatus] Human-readable load/empty status.
 * @param {HTMLElement} [options.zoomInButton]
 * @param {HTMLElement} [options.zoomOutButton]
 * @param {HTMLElement} [options.resetButton]
 */
export function createTownSquareMap({
  root,
  mode = "full",
  ...
  let active = true; // paused while off-screen/hidden in embedded mode
  let destroyed = false;
  ...
  function buildMap() {
    svg = createSvgElement("svg", {
      class: `map-svg${mode === "embedded" ? " map-svg--compact" : ""}`,
      role: "group",
    ...
    onStatus(`${sites.length} verified TownSquare${sites.length === 1 ? "" : "s"} on the map.`);
  ...
  function renderSiteNode(site) {
    ...
    // Compact/embedded rendering keeps labels on the biggest towns visible by
    // default (see .map-svg--compact in map.css) and hides the rest until
    // hover/focus/selection, so a small canvas doesn't turn into label soup.
    const isMajor = marker.tier.radius >= 32;
    const group = createSvgElement("g", {
      class: `map-node${isMajor ? " map-node--major" : ""}${site.inactiveFor30Days ? " is-inactive" : ""}${site.siteKey === selectedSiteKey ? " is-selected" : ""}`,
  ...
  async function refreshActivity() {
    if (!active || document.hidden || activityRefreshInFlight) return;
    activityRefreshInFlight = true;

    try {
      const response = await fetch(`${fetchBase}/api/map/activity`, { mode: fetchBase ? "cors" : "same-origin", credentials: "omit" });
      const body = await response.json();
      if (!response.ok || !Array.isArray(body.sites)) return;

      if (body.version !== structureVersion) {
        const mapResponse = await fetch(`${fetchBase}/api/map`, { mode: fetchBase ? "cors" : "same-origin", credentials: "omit" });
        ...
        mapLoadFailed = false;
        onStats(computeNetworkStats(sites));
  ...
  function renderSelectedState() {
    ...
    root.querySelectorAll(".map-link").forEach((edge) => {
      ...
      const isActive = selectedSiteKey && (fromKey === selectedSiteKey || toKey === selectedSiteKey);
  ...
  function scheduleInitialViewFit() {
    requestAnimationFrame(() => {
      ...
      resizeObserver = new ResizeObserver(() => {
        if (fitInitialView() && resizeObserver) {
  ...
  const onZoomIn = () => zoomAt(ZOOM_STEP);
  const onZoomOut = () => zoomAt(1 / ZOOM_STEP);
  ...
  async function loadMap() {
    try {
      const response = await fetch(`${fetchBase}/api/map`, { mode: fetchBase ? "cors" : "same-origin", credentials: "omit" });
      const body = await response.json();
  ...
  wireControls();
  void loadMap();
  activityTimer = window.setInterval(refreshActivity, activityRefreshMs);
  ...
  return {
    ...
    refreshNow: refreshActivity,
    visitRandomTown: (excludeSiteKey = "") => fetchRandomTown(excludeSiteKey, fetchBase),
    reportVisit: (siteKey) => reportMapClick(siteKey, fetchBase),
    destroy() {
      destroyed = true;
