...
| 2026-06-25 | T2 🟡 | Added `.github/workflows/ci.yml` running `npm ci` + `npm run check` + `npm test` on push/PR (closes the "no CI" gap). **Remaining:** extract pure helpers (token compare, sanitizers, origin/config validation, identity) from `server.js` into testable modules and add `node --test` suites; make `scripts/smoke-test.js` self-contained (it currently needs an already-running server + specific env, and fails identically on `main`). |
