...
| 2026-06-25 | H1 🟡 (in progress) | Server monolith decomposition is underway: extracted `server/auth-tokens.js`, `server/sanitize.js`, and `server/admin-sessions.js` out of `server.js`, and fixed the static-file path guard (H8) in place. **Remaining extractions** (each its own focused, smoke-validated step): `server/static-files.js` (resolvePublicFile + headers + dev/staging gating — note Plausible-inject coupling), `server/plausible.js` (script/event proxy — fold in the H9 rate-limit fix), `server/rate-limit.js` (recentBucket / auth-failure / IP-event budgets + their Maps), `server/scene.js` (scene/identity/bird simulation), `server/ws-handlers.js`, `server/admin-routes.js`, `server/sites-store.js` (persistence). These touch shared module-level mutable state (`sitesByKey`, `scenes`, rate-limit Maps, runtime-loaded constants), so each needs careful dependency threading and is **not** fully covered by the current smoke tests — do them one at a time. |
